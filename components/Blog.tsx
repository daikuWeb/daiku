...
export function Blog() {
  ...
  const getPageItems = () => {
    ...
    if (isAdmin && currentPage === 1) {
      // Untuk page pertama admin, ambil 2 blog karena ada tombol tambah
      return sortedBlogs.slice(0, 2);
    } else {
      // Untuk page lain, ambil 3 blog
      const startIndex = isAdmin 
        ? 2 + ((currentPage - 2) * ITEMS_PER_PAGE) // Mulai dari index 2 untuk admin setelah page 1
        : (currentPage - 1) * ITEMS_PER_PAGE;
      return sortedBlogs.slice(startIndex, startIndex + ITEMS_PER_PAGE);
    }
  };

  const getTotalPages = () => {
    if (isAdmin) {
      // Kurangi 2 dari total blog dan tambahkan tombol tambah sebagai 1 item
      const remainingItems = blogs.length - 2;
      return Math.ceil((remainingItems + 3) / ITEMS_PER_PAGE);
    } else {
      return Math.ceil(blogs.length / ITEMS_PER_PAGE);
    }
  };
            ...
            <Link href={`/blog/${post.id}`} key={post.id} className="block transform transition-all duration-300 hover:-translate-y-1">
              <article className="relative bg-white rounded-3xl overflow-hidden shadow-lg h-full flex flex-col">
                <div className="relative h-[240px] w-full flex-shrink-0">
                  ...
                  />
                  {/* Garis kuning di bawah gambar */}
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-yellow-500 z-10"></div>
                </div>
